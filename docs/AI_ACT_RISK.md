...
| **Art. 50(4): Deep fake disclosure — clones from a *custom reference clip*** | The row above was true for a baked voice GGUF and false for the runtime path beside it: the Synthesize screen never passed `customVoiceWavPath` to `writeWav`, so a clone made from a user-supplied reference WAV — the route the voice-clone wizard uses, and the one a first-run user reaches — was written **without** the beep. The parameter is now passed on that path, and the wizard hand-off is covered by `test/voice_clone_handoff_test.dart` | Done (2026-08-29) |
...
| Date | Change |
|---|---|
| 2026-08-29 | **One Art. 50(4) defect, found by a user walking the app rather than by an audit.** Issue #35 is a first-run report (write-up: `HISTORY.md` §18), and most of it is not regulatory — corrupted download resumes, a language filter with no data behind it, duplicate voice rows, an onboarding pass that played a different voice than it downloaded, screens with no back button. One finding belongs here: **cloned speech generated from a custom reference clip carried no beep disclaimer.** §5.2 asserted the mandatory beep applied on the GUI, server and CLI cloning paths, and it did — for a *baked* voice GGUF. The Synthesize screen never passed `customVoiceWavPath` to `writeWav`, so the runtime-cloning route immediately beside it, which is the one the voice-clone wizard drives and therefore the one a new user reaches first, produced unmarked deep-fake audio. This is the same shape recorded seven times above — a duty implemented on the surface it was designed on and missed on a second route to the same capability — with one new twist worth stating: the second route here was not obscure or latent, it was **the default path for the feature's own wizard**. What made it invisible is that no one had used the wizard end to end; the hand-off from wizard to screen was itself broken (`go()` re-uses a page keyed by path, so `initState` never re-ran and the clip was dropped), so every internal test of cloning went through the manual, baked-voice route that was correctly marked. The standing instruction gains a third question after "on which surfaces?" and "which build targets?": **which route does a first-run user actually take, and has anyone walked it on a real build?** `PLAN.md` §B6 makes that walkthrough a release step. |
