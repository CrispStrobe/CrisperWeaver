...
| Web      | 🆕    | Flutter web platform scaffolded with conditional imports (`lib/native/` stubs for crispasr/crispembed/ffi). PWA manifest with branded icons, `standalone` display mode. Deployed to Vercel. No native FFI — feature set limited to mock engine / remote server mode. |
